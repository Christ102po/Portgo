...
export function Sidebar() {
  ...
  return (
    <aside className="sticky top-0 flex h-screen w-64 shrink-0 flex-col justify-between border-r border-emerald-950/40 bg-emerald-900 text-white">
      ...
      <div className="border-t border-white/10 px-3 py-4">
        <div className="mb-2 px-4">
          ...
          <p className="truncate text-xs text-white/50">{admin?.role?.replace("_", " ")}</p>
        </div>
        <button
          onClick={logout}
          className="flex w-full items-center gap-3 rounded-full px-4 py-2.5 text-sm font-semibold text-red-400 transition-colors hover:bg-red-500/10 hover:text-red-300"
        >
